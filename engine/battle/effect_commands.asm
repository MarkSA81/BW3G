...
.not_asleep
	...
	; Flame Wheel, Flare Blitz, Scald thaw the user.
	ld a, [wCurPlayerMove]
	cp FLAME_WHEEL
	jr z, .not_frozen
	cp FLARE_BLITZ
...
.Unevolved:
; check if defender has evolutions
...
; hl := the species' entry from EvosAttacksPointers
	...
	call GetFarHalfword
; a := first byte of species' *EvosAttacks data
	ld a, BANK("Evolutions and Attacks")
	call GetFarByte
; if a == 0, no evolutions, so don't boost
	and a
	pop bc
	pop hl
	ret z
	
; check if defender's item is eviolite
	push bc
	call GetOpponentItem
	ld a, b
	cp HELD_EVIOLITE
...
; boost relevant defense stat in bc by 50%
	ld a, c
	srl a
	add c
	ld c, a
...
INCLUDE "engine/battle/move_effects/hammer_arm.asm"
